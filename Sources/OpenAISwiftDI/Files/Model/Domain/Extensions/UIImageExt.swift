...
extension UIImage {
    ...
    public func processPixels(location: CGPoint?,
                              color: UIImage.RGBA32 = .clear, size: Int = 20) -> UIImage? {
...
//            print("unable to get context data")
        ...
        if let location = location {
            ...
            let yPoint = Int(location.x)
            let xPoint = Int(location.y)
            let xMin = xPoint - size
            let xMax = xPoint + size
            let yMin = yPoint - size
            let yMax = yPoint + size

            guard xMin >= 0, xMax <= width + size * 2,
                    yMin >= 0, yMax <= height + size * 2 else {
                return self
            }
            ...
            for yIdx in yRange {
                for xIdx in xRange {
                    let offset = xIdx * width + yIdx
                    if offset <= (width * height) &&
                        offset >= 0 && pixelBuffer[offset].alphaComponent > 0 {
...
extension UIImageValueTransformer {
    ...
    /// This is the name used to register the transformer using
